...
if table in tables['name'].values:
    ...
    if res:
        ...
        if (start_year < end_year) or (start_year == end_year and start_month < end_month):
            print(f"Actualizando datos mensuales desde {start_year}-{start_month}...")
        ...
        if (start_year == end_year and start_month == end_month and start_day < end_day):
            print(f"Actualizando datos diarios desde {start_year}-{start_month}-{start_day}...")
            agg_trades_daily(end_year, end_month, start_day, end_day, simbol)
    else:
        # Si la tabla está vacía, empezar desde 2025
        print("Tabla existente pero vacía. Iniciando descarga desde 2025-01.")
        agg_trades_monthly(2025, 1, end_year, end_month, end_day, simbol)
else:  
    print(f"Base de datos no encontrada. Iniciando descarga completa hasta: {end_year}-{end_month}-{end_day-1}")
    agg_trades_monthly(2025, 1, end_year, end_month, end_day, simbol)

# --- POST-PROCESAMIENTO ---
# Descomentar si se desea regenerar las tablas de análisis después de la descarga
# print("\n[ANÁLISIS] Regenerando tablas de OHLC y Volume Profile...")
